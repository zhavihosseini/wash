...
@else
    <div class="container" style="overflow-x: scroll">
        <h3 style="text-align: center">Multiple Deleted <strong>REPLY</strong> Row</h3>
        <button style="margin-bottom: 10px;" class="btn btn-danger delete_all center-block justify-content-center" data-url="{{ url('myproductDeleterep') }}">Delete All Selected</button>
            ...
            @if($allrep->count())
                @foreach($paginate as $key => $product)
                    <tr id="tr_{{$product->id}}">
                        ...
                        <td>{{$product->id}}</td>
                        <td>{{ ++$key }}</td>
                        <td style="direction: rtl;font-family: Yekan">{{ $product->text }}</td>
                        <td style="direction: rtl;font-family: Yekan">{{ $product->time}}</td>
                        <td>{{ $product->userid}}</td>
                        <td style="direction: rtl;font-family: Yekan">{{ $product->time}}</td>
                        <td>{{ $product->code}}</td>
                        <td>
                            <a href="{{ url('myproductrep',$product->id) }}" class="btn btn-danger btn-sm"
                        ...
                        <td>
                            <form action="<?php echo route('viewrep')?>" method="get">
                                <?php echo csrf_field();?>
